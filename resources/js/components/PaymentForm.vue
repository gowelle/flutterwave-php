...
<style scoped>
.flw-payment-form { max-width: 480px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; }
.flw-alert { display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1rem; border-radius: 0.5rem; margin-bottom: 1rem; }
.flw-alert-error { background: #fef2f2; border: 1px solid #fecaca; color: #dc2626; }
.flw-alert-icon { width: 1.25rem; height: 1.25rem; flex-shrink: 0; }
.flw-form-section { margin-bottom: 1.5rem; }
.flw-form-section-title { font-size: 0.875rem; font-weight: 600; color: #374151; margin-bottom: 1rem; text-transform: uppercase; letter-spacing: 0.05em; }
.flw-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; }
.flw-form-grid-3 { grid-template-columns: 1fr 1fr 1fr; }
.flw-form-group { margin-bottom: 1rem; }
.flw-label { display: block; font-size: 0.875rem; font-weight: 500; color: #374151; margin-bottom: 0.375rem; }
.flw-input { width: 100%; padding: 0.75rem 1rem; border: 1.5px solid #d1d5db; border-radius: 0.5rem; font-size: 1rem; transition: border-color 0.15s; background: white; }
.flw-input:focus { outline: none; border-color: #f5a623; box-shadow: 0 0 0 3px rgba(245, 166, 35, 0.1); }
.flw-input-with-icon { position: relative; }
.flw-card-brand-icon { position: absolute; right: 0.75rem; top: 50%; transform: translateY(-50%); height: 1.5rem; width: auto; }
.flw-payment-summary { display: flex; justify-content: space-between; background: #f9fafb; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem; }
.flw-amount-label { color: #6b7280; font-weight: 500; }
.flw-amount-value { font-size: 1.25rem; font-weight: 700; color: #111827; }
.flw-btn { display: flex; align-items: center; justify-content: center; padding: 0.875rem 1.5rem; border-radius: 0.5rem; font-size: 1rem; font-weight: 600; cursor: pointer; border: none; transition: all 0.15s; }
.flw-btn-primary { background: linear-gradient(135deg, #f5a623, #f77f00); color: white; }
.flw-btn-primary:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(245, 166, 35, 0.4); }
.flw-btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
.flw-btn-full { width: 100%; }
.flw-btn-loading { pointer-events: none; }
.flw-security-notice { display: flex; align-items: center; justify-content: center; gap: 0.5rem; margin-top: 1rem; font-size: 0.75rem; color: #6b7280; }
.flw-security-icon { width: 1rem; height: 1rem; }
@media (max-width: 480px) { .flw-form-grid, .flw-form-grid-3 { grid-template-columns: 1fr; } }
</style>
