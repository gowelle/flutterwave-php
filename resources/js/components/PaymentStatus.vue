...
import type { PaymentStatusProps, DirectChargeResponse } from '../types';
...
const statusMessage = ref('Processing your payment...');
...
function updateFromCharge(chargeData: DirectChargeResponse) {
  ...
  statusMessage.value = {
    ...
    requires_action: 'Awaiting authorization...',
    succeeded: 'Payment successful!',
    failed: chargeData.issuer_response?.message || 'Payment failed.',
    cancelled: 'Payment was cancelled.',
    timeout: 'Payment authorization timed out.',
  }[chargeData.status] || 'Processing...';
    ...
    <div class="flw-status-container">
      ...
      <div class="flw-status-actions">
        <slot name="actions" :status="status" :charge="charge">
          <button v-if="status === 'succeeded'" class="flw-btn flw-btn-success flw-btn-full">Continue</button>
          <button v-else-if="isTerminal" class="flw-btn flw-btn-primary flw-btn-full">Try Again</button>
        </slot>
      </div>
    </div>
  </div>
...
.flw-payment-details { background: #f9fafb; border-radius: 0.75rem; padding: 1rem; margin-bottom: 1.5rem; text-align: left; }
.flw-detail-row { display: flex; justify-content: space-between; padding: 0.5rem 0; color: #6b7280; }
.flw-detail-value { font-weight: 600; color: #111827; }
.flw-polling-indicator { margin-bottom: 1.5rem; }
.flw-polling-dots { display: flex; justify-content: center; gap: 0.25rem; margin-bottom: 0.5rem; }
.flw-polling-dots span { width: 0.5rem; height: 0.5rem; background: #f5a623; border-radius: 50%; animation: bounce 1.4s infinite ease-in-out; }
.flw-polling-dots span:nth-child(1) { animation-delay: -0.32s; }
.flw-polling-dots span:nth-child(2) { animation-delay: -0.16s; }
