...
<template>
  <div class="flw-pin-input">
    <div class="flw-pin-header">
      ...
      <p class="flw-pin-subtitle">Please enter your {{ pinLength }}-digit card PIN</p>
    </div>
    ...
    <button type="button" class="flw-btn flw-btn-primary flw-btn-full" :disabled="!isPinComplete || processing" @click="submitPin">
      <span v-if="!processing">Confirm PIN</span>
...
<style scoped>
.flw-pin-input { max-width: 360px; margin: 0 auto; padding: 2rem; text-align: center; }
.flw-pin-header { margin-bottom: 2rem; }
.flw-pin-icon { width: 4rem; height: 4rem; margin: 0 auto 1rem; padding: 1rem; background: linear-gradient(135deg, #fef3c7, #fbbf24); border-radius: 50%; color: #92400e; }
.flw-pin-icon svg { width: 100%; height: 100%; }
.flw-pin-title { font-size: 1.25rem; font-weight: 700; color: #111827; margin-bottom: 0.5rem; }
.flw-pin-subtitle { font-size: 0.875rem; color: #6b7280; }
.flw-pin-boxes { display: flex; justify-content: center; gap: 0.75rem; margin-bottom: 1.5rem; }
.flw-pin-box { width: 3rem; height: 3.5rem; text-align: center; font-size: 1.5rem; font-weight: 700; border: 2px solid #d1d5db; border-radius: 0.5rem; background: white; }
.flw-pin-box:focus { outline: none; border-color: #f5a623; box-shadow: 0 0 0 3px rgba(245, 166, 35, 0.2); }
...
.flw-cancel-link { display: block; margin-top: 1rem; font-size: 0.875rem; color: #6b7280; background: none; border: none; cursor: pointer; text-decoration: underline; }
